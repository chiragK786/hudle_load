...
class APITestUser(HttpUser):
    ...
    def generate_otp(self):
        ...
        with self.client.post('/api/v1/otp/new', json=json_body, headers=headers, catch_response=True) as response:
            ...
            assert response.elapsed.total_seconds() * 1000 < self.MAX_RESPONSE_TIME_MS, "OTP generation API took too long!"
    ...
    def get_cities(self):
        ...
        with self.client.get('/api/v1/cities', headers=headers, catch_response=True) as response:
            ...
            assert response.elapsed.total_seconds() * 1000 < self.MAX_RESPONSE_TIME_MS, "Cities API took too long!"
...
def _post(self, endpoint, json_body=None, files=None, headers=None):
    if headers is None:
        headers = {
            'Api-Secret': self.api_key,
...
if __name__ == "__main__":
    ...
    os.environ["LOCUST_RUN_TIME"] = "1m"
    os.environ["LOCUST_HOST"] = "https://example.com"
    os.system("locust -f locust test.py")